...
Brazilian Portuguese translation of `lead-generation-scraping-apis`, published as `github.com/automationbyexperts/apis-scraping-geracao-de-leads`
(local folder `all_public_actors/APIFY_GITHUB/apis-scraping-geracao-de-leads`). Same affiliate model: every Apify link
...
- A regional group sits **first** in `groups` (`brasil`: Brasil e Portugal), so a local tool is
